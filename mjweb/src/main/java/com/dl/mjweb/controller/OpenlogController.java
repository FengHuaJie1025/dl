...
public class OpenlogController extends BaseController  {
	...
	@Autowired
	private final IOpenlogService openlogService;

	@GetMapping(value = "/list")
	@ApiOperation("get All DlOpenlog")
	public DlResponse list() {
		List<DlOpenlog> list = openlogService.findAll();
		return DlResponse.ok(list);
	}
	
	@GetMapping(value="page")
	@ApiOperation("page DlOpenlog")
	public DlResponse page(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "20") int pageSize) {
		...
		Specification<DlOpenlog> spec = QueryUtil.buildSpec(DlOpenlog.class,map,"pid");
		Page page=  openlogService.page(spec,pageable);
		return DlResponse.ok(page);
	}

	@PostMapping
	@OperationLogAnn("add DlOpenlog")
	@ApiOperation("add DlOpenlog")
	 public DlResponse add(@RequestBody DlOpenlog bean, HttpServletRequest request) {
		openlogService.save(bean);
		return DlResponse.ok(bean);
    }

	@PutMapping(value = "/{id}")
	@OperationLogAnn("update DlOpenlog")
	@ApiOperation("update DlOpenlog By id")
	public DlResponse update(@PathVariable("id")int id,@RequestBody DlOpenlog bean,HttpServletRequest request) {
		bean.setId(id);
		openlogService.update(bean);
		return DlResponse.ok(bean);
    }
	
	@GetMapping(value = "/{id}")
	@ApiOperation("get DlOpenlog By id")
	public DlResponse get(@PathVariable("id")int id) {
		DlOpenlog openlog = openlogService.getById(id);
		return DlResponse.ok(openlog);
	}
	
	@DeleteMapping(value = "/{id}")
	@OperationLogAnn("delete DlOpenlog")
	@ApiOperation("delete DlOpenlog By id")
	public DlResponse delete(@PathVariable("id")int id) {
		openlogService.delete(id);
		return DlResponse.ok(null);
	}

//	private Specification<DlOpenlog> buildSpecByQuery() {
//		HashMap<String,String> map = getRequestParamMap();
//
//		return (Specification<DlOpenlog>) (root, query, criteriaBuilder) -> {
